...
io.on("connection", (socket) => {
  ...
  socket.on("fetchData", async (promptIndex, callback) => {
    const db = mongo.db("PhiScript-Generics");

    const cursor = db.collection("Points").find({ promptIndex: promptIndex });

    let data = [];
    const addToDataArray = (d) => {
      data.push(d);

      // d.data.points.forEach((point) => {
      //   data.push(point);
      // });
    };

    await cursor.forEach(addToDataArray);
